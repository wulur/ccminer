	...
	uint32_t *hashes_done)
{
	...
	if(datasize == 200)
	{
		...
		applog(LOG_ERR, "datasize = %d !!!", datasize);

	do
	{
		pascal_cpu_hash(thr_id, throughput, pdata+(datasize/64*16), datasize%64, ms, h_nounce);
		if(stop_mining) {mining_has_stopped[thr_id] = true; cudaStreamDestroy(gpustream[thr_id]); pthread_exit(nullptr);}
		if(h_nounce[0] != 0)
		{
			uint32_t vhash64[8]={0};
			pascal_hash(vhash64, pdata, h_nounce[0], ms);
			if (!opt_verify || (vhash64[7] == 0 && fulltest(vhash64, ptarget)))
			{
				...
				*hashes_done = pdata[19] - first_nonce + throughput;
				if (h_nounce[1] != 0)
				{
					pascal_hash(vhash64, pdata, h_nounce[1], ms);
					if (!opt_verify || (vhash64[7] == 0 && fulltest(vhash64, ptarget)))
					{
						pdata[21] = h_nounce[1];
						res++;
